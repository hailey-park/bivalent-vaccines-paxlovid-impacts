...
bivalent_booster_data <- read.csv("outcome data clean/bivalent_booster_doses_clean_sensitivity_extreme_uptake.csv")[,-1]
...
for (i in c(1:n)) {
  ...
  predictions_df <- merge(merge(covid_df %>% group_by(age_group, boost_2_vax_status_match) %>% 
    ...
    mutate(predicted_outcomes = round(predicted_outcomes),
           max_uptake = case_when(age_group %in% c("0-17 years", "18-49 years", "50-64 years") ~ 0,
                                  age_group %in% c("65-74 years", "75-84 years") ~ 0.8,
                                  age_group %in% c("85+ years") ~ 0.9,
                                  TRUE ~ 0),
...
saveRDS(vax_hosp_results, "data/results-sensitivity-extremeUptake-vax-hosp.RDS")
####################################################
...
for (i in c(1:n)) {
  ...
  predictions_pax_df <- merge(covid_hosp 
    ...
    mutate(baseline_pax_uptake = case_when(age_group == "50-64 years" ~ 0.237,
                                           ...
                                           TRUE ~ 0),
           max_pax_uptake = case_when(age_group == "50-64 years" ~ 0.237,
                                      age_group == "85+ years" ~ 0.9,
                                      age_group %in% c("65-74 years", "75-84 years") ~ 0.8,
                                      TRUE ~ 0))
  ...
  combined_strat_outcomes <- combined_strat %>% group_by(strategy) %>% summarise(averted_outcomes_diff = ceiling(sum(averted_hosp_diff)),
                                                                                 total_outcomes = ceiling(sum(total_predicted)),
    ...
    mutate(averted_outcomes_cascade = ceiling(averted_outcomes_diff * 0.8 * 0.895),
           ...
           perc_averted = round(averted_outcomes_cascade/ total_outcomes * 100, 1),
...
saveRDS(pax_hosp_results, "data/results-sensitivity-extremeUptake-pax-hosp.RDS")
...
saveRDS(vax_cases_results, "data/results-sensitivity-extremeUptake-vax-cases.RDS")

#################################################################
#Evaluate the 95% UIs for the results
results_pax_death <- readRDS("data/results-sensitivity-extremeUptake-pax-death.RDS")
results_pax_hosp <- readRDS("data/results-sensitivity-extremeUptake-pax-hosp.RDS")
results_vax_hosp <- readRDS("data/results-sensitivity-extremeUptake-vax-hosp.RDS")
results_vax_cases <- readRDS("data/results-sensitivity-extremeUptake-vax-cases.RDS")
results_vax_death <- readRDS("data/results-sensitivity-extremeUptake-vax-death.RDS")
