...
predictions_df <- merge(merge(validation_period %>% group_by(weeks_since_july2022, age_group, boost_2_vax_status_match) %>% 
                          summarise(num_death = sum(num_death_adj),
                                    predicted_death = sum(prediction)) %>% 
  ...
  mutate(predicted_death = round(predicted_death)), ve_estimates, by.x = "boost_2_vax_status_match", by.y = "baseline_vacc", all.x = TRUE)
