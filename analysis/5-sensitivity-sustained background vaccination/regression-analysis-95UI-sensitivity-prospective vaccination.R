...
bivalent_booster_data <- read.csv("outcome data clean/bivalent_booster_doses_clean_sensitivity_prospective vaccination.csv")[,-1]
...
for (i in c(1:n)) {
  ...
  #merge together
  predictions_df <- merge(merge(covid_df %>% 
                            mutate(vax_coverage = case_when((weeks_since_july2022 %in% c(0:4)) ~ 0.033,
                                                            (weeks_since_july2022 %in% c(5:8)) ~ 0.066,
                                                            (weeks_since_july2022 %in% c(9:13)) ~ 0.099,
                                                            (weeks_since_july2022 %in% c(14:18)) ~ 0.1333,
                                                            (weeks_since_july2022 %in% c(19:22)) ~ 0.166,
                                                            (weeks_since_july2022 %in% c(23:26)) ~ 0.198,
                                                            TRUE ~ 0)),
  ...
  strat1 <- predictions_df %>% mutate(averted_diff = ifelse((boost_2_vax_status_match %in% c("Unvaccinated", "Primary Series", "Boosted (1 dose)", "Boosted (2 doses)")),
                                                            predicted_outcomes * ve_hosps * (0.7 - bivalent_coverage - vax_coverage), 0),
                                      total_predicted = predicted_outcomes * (1 - ((bivalent_coverage + vax_coverage) * ve_hosps)))
  ...
  combined_strat_outcomes <- combined_strat %>% mutate(averted_diff = ifelse(averted_diff < 0, 0, averted_diff)) %>% group_by(strategy) %>% summarise(averted_outcomes_diff = round(sum(averted_diff)),
...
saveRDS(vax_hosp_results, "data/results-prospective-vax-hosp.RDS")
...
for (i in c(1:n)) {
  ...
  predictions_df <- merge(covid_cases %>% mutate(ve_cases = case_when((weeks_since_july2022 %in% c(0:4) & boost_2_vax_status_match == 'Unvaccinated') ~ ve$unvax_0_to_4,
                                                                      ...
                                                                      TRUE ~ 0),
                                                 vax_coverage = case_when((weeks_since_july2022 %in% c(0:4)) ~ 0.033,
                                                                          (weeks_since_july2022 %in% c(5:8)) ~ 0.066,
                                                                          (weeks_since_july2022 %in% c(9:13)) ~ 0.099,
                                                                          (weeks_since_july2022 %in% c(14:18)) ~ 0.1333,
                                                                          (weeks_since_july2022 %in% c(19:22)) ~ 0.166,
  ...
  #simulate strategies
  strat1 <- predictions_df %>% mutate(averted_diff = ifelse((boost_2_vax_status_match %in% c("Unvaccinated", "Primary Series", "Boosted (1 dose)", "Boosted (2 doses)")),
                                                            predicted_cases * ve_cases * (0.7 - bivalent_coverage - vax_coverage), 0),
                                      total_predicted = predicted_cases * (1 - ((bivalent_coverage + vax_coverage) * ve_cases)))
...
saveRDS(vax_cases_results, "data/results-prospective-vax-cases.RDS")

######################################################

#Evaluate the 95% UIs for the results
results_prospective_vax_cases <- readRDS("data/results-prospective-vax-cases.RDS")
...
inspection <- results_prospective_vax_cases %>% apply(2, quantile, probs=c(0.025, 0.975))
