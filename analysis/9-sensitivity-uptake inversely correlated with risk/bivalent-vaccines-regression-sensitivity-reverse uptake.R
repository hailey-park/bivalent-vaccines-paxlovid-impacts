###################################################################################################
#Title: Bivalent Booster Regression Analysis (reverse uptake)
#Author: Hailey Park
#Date: July 3, 2023
###################################################################################################
rm(list=ls())
...
predictions_df <- merge(merge(validation_period %>% group_by(age_group, boost_2_vax_status_match) %>% 
                                summarise(num_death = sum(num_death_adj), predicted_death = sum(prediction)),
  ...
  select(-num_death) %>% mutate(predicted_death = round(predicted_death))
...
##########################
#Adding age-stratified uptake 

predictions_df <- predictions_df %>% mutate(max_uptake = case_when(age_group %in% c("0-17 years", "18-49 years", "50-64 years") ~ 0.7,
...
strat1 <- predictions_df %>% mutate(averted_diff = ifelse((boost_2_vax_status_match %in% c("Unvaccinated", "Primary Series", "Boosted (1 dose)", "Boosted (2 doses)")),
                                                          predicted_death * ve_deaths * (max_uptake - bivalent_coverage), 0),
                                    total_predicted = predicted_death * (1 - (bivalent_coverage * ve_deaths)))
