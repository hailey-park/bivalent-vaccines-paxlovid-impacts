...
bivalent_booster_data <- read.csv("outcome data clean/bivalent_booster_doses_clean_perfectuptake.csv")[,-1]
...
for (i in c(1:n)) {
  ...
  strat1 <- predictions_df %>% mutate(averted_diff = ifelse((boost_2_vax_status_match %in% c("Unvaccinated", "Primary Series", "Boosted (1 dose)", "Boosted (2 doses)")),
                                                            predicted_outcomes * ve_hosps * (1 - bivalent_coverage), 0),
...
saveRDS(vax_hosp_results, "data/results-sensitivity-perfect uptake-vax-hosp.RDS")
####################################################
#Paxlovid Simulations

#Create df to store results
pax_hosp_results <- data.frame(matrix(nrow = 1000, ncol = 3*4))
colnames(pax_hosp_results) <- c(sprintf("strat%d_nnt", 1:4),
                                 sprintf("strat%d_perc", 1:4),
                                 sprintf("strat%d_total", 1:4))
pax_death_results <- data.frame(matrix(nrow = 1000, ncol = 3*4))
colnames(pax_death_results) <- c(sprintf("strat%d_nnt", 1:4),
                                sprintf("strat%d_perc", 1:4),
                                sprintf("strat%d_total", 1:4))
...
for (i in c(1:n)) {
  ...
  predictions_pax_df <- merge(covid_death 
    ...
    mutate(baseline_pax_uptake = case_when(age_group == "50-64 years" ~ 0.237,
           ...
           max_pax_uptake = case_when(age_group == "18-49 years" ~ 0.02,
                                      ...
                                      age_group %in% c("65-74 years", "75-84 years", "85+ years") ~ 1,
                                      TRUE ~ 0))
  ...
  pax_death_results[i,] <- c(combined_strat_outcomes$NNT, combined_strat_outcomes$perc_averted, combined_strat_outcomes$averted_outcomes_cascade)
}

saveRDS(pax_death_results, "data/results-sensitivity-perfect uptake-pax-death.RDS")
...
for (i in c(1:n)) {
  ...
  #simulate strategies
  strat1 <- predictions_df %>% mutate(averted_diff = ifelse((boost_2_vax_status_match %in% c("Unvaccinated", "Primary Series", "Boosted (1 dose)", "Boosted (2 doses)")),
                                                            predicted_cases * ve_cases * (1 - bivalent_coverage), 0),
...
saveRDS(vax_cases_results, "data/results-sensitivity-perfect uptake-vax-cases.RDS")


#################################################################
#Evaluate the 95% UIs for the results
results_pax_death <- readRDS("data/results-sensitivity-perfect uptake-pax-death.RDS")
results_pax_hosp <- readRDS("data/results-sensitivity-perfect uptake-pax-hosp.RDS")
results_vax_hosp <- readRDS("data/results-sensitivity-perfect uptake-vax-hosp.RDS")
results_vax_cases <- readRDS("data/results-sensitivity-perfect uptake-vax-cases.RDS")
results_vax_death <- readRDS("data/results-sensitivity-perfect uptake-vax-death.RDS")
