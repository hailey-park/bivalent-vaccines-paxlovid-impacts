...
for (i in 1:1000){
  ...
  strat5 <- predictions_df %>% mutate(averted_diff = ifelse(((age_group %in% c( "75-84 years", "85+ years")) & (boost_2_vax_status_match != "Unvaccinated")),
                                                            predicted_cases * ve_cases * (i/1000) *(0.7 - bivalent_coverage), 0)) 
  strat5$strategy <- rep("strat5", nrow(strat5))
  

  
  combined_strat <- bind_rows(strat2, strat5)
  ...
  merged_data <- merge(combined_strat_outcomes, bivalent_booster_data, by = c("strategy"), all.X = TRUE) %>%
    mutate(ARR = averted_outcomes_diff/(total_doses),
           NNT = ceiling(1/ARR),
           strategy = factor(strategy, levels = c("strat2", "strat5")))%>%
    select(-c(variable))
  
  results[i,"NNT_diff"] <- merged_data$NNT[1] - merged_data$NNT[2]
  
}

write.csv(results, "sensitivity-reduction in effectiveness/data/results-vax-cases.csv")
