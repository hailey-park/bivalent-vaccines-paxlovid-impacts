...
model1 <- glm(num_hosp_adj ~ age_group + boost_2_vax_status_match, 
...
sum(calibration_period$num_hosp_adj)
sum(calibration_period$prediction_hosp)
###################################################################################################
...
results <- data.frame(reduction = 1:1000,
                      NNT_diff = rep(0, 1000))
...
for (i in 1:1000){
  ...
  strat3 <- predictions_pax_df %>% mutate(averted_hosp_diff = case_when((age_group %in% c("18-49 years",
                                                                                          "50-64 years",
                                                                                          "65-74 years", 
                                                                                          "75-84 years",
                                                                                          "85+ years")) ~ predicted_hosp * pe_hosps * (max_pax_uptake - baseline_pax_uptake),
                                                                        TRUE ~ 0),
  ...
  strat6 <- predictions_pax_df %>% mutate(averted_hosp_diff = ifelse((age_group %in% c("75-84 years", "85+ years")),
                                                                     predicted_hosp * pe_hosps * (i/1000) * (max_pax_uptake - baseline_pax_uptake), 0),
  ...
  combined_strat <- bind_rows(strat3, strat6)
  combined_strat_outcomes <- combined_strat %>% group_by(strategy) %>% summarise(averted_outcomes_diff = ceiling(sum(averted_hosp_diff)),
    ...
    mutate(averted_outcomes_cascade = ceiling(averted_outcomes_diff * 0.8 * 0.895),
           ...
           strategy = factor(strategy, levels = c("strat3",  "strat6")))
  
  results[i,"NNT_diff"] <- combined_strat_outcomes$NNT[1] - combined_strat_outcomes$NNT[2]
  
  }

#######################################
write.csv(results, "sensitivity-reduction in effectiveness/data/results-pax-hosp.csv")
