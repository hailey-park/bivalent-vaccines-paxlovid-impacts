...
bivalent_coverage_with_old <- rbind(bivalent_coverage_prev, oldest_age) %>% filter(age_group != "65+") %>%
...
write.csv(adjustment_factor_clean, "outcome data clean/adjustment_factor_matrix_cases.csv")
...
write.csv(bivalent_coverage_age_vax_clean, "outcome data clean/bivalent_coverage_age_vax.csv")
