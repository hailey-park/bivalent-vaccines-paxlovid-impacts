###################################################################################################
#Title: Bivalent Booster Regression Analysis-Stratified Risk Groups
#Author: Hailey Park
#Date: July 3, 2023
###################################################################################################
rm(list=ls())
...
covid_case <- merge(merge(covid_case_data %>% mutate(weeks_since_july2022 = as.integer(floor(difftime(strptime(dtepisode, format="%Y-%m-%d"), 
                                                                                                       strptime("2022-07-23", format="%Y-%m-%d"), units = 'weeks')))) %>%
                             group_by(weeks_since_july2022, age_group, boost_2_vax_status_match, .drop = FALSE) %>% summarise(num_case = n()),
  ...
  mutate(num_case_adj = ceiling(num_case * adj_factor)) %>% select(weeks_since_july2022, boost_2_vax_status_match, age_group, num_case, adj_factor, num_case_adj)
...
model1 <- glm(num_case_adj ~ age_group + boost_2_vax_status_match,
              data = calibration_data, "quasipoisson")
summary(model1)


#Model Calibration Check (Predicting on 6 months of data that it was calibrated on)
validation_period <- covid_case %>% filter(weeks_since_july2022 %in% c(0:26))
...
predictions_df <- merge(merge(validation_period %>% group_by(age_group, boost_2_vax_status_match) %>% 
                                summarise(num_case = sum(num_case_adj), predicted_case = sum(prediction)),
  ...
  select(-num_case) %>% mutate(predicted_case = round(predicted_case))
...
predictions_df <- merge(validation_period %>% group_by(weeks_since_july2022, age_group, boost_2_vax_status_match) %>% 
                          summarise(num_case = sum(num_case_adj),
                                    predicted_case = sum(prediction)) %>% 
...
##########################
#Vaccine Strategies Analysis (Stratified Analysis: Strategies 8-16)
